...
export const calculateHabitStatus = (habit, log) => {
    if (!log) {
...
const calculateCountStatus = (habit, log) => {
...
const calculateDurationStatus = (habit, log) => {
    const value = log.value || 0;
    const target = habit.dailyTarget || 1;
    const comparison = habit.dailyTargetComparison || HabitTargetComparison.GREATER_THAN_OR_EQUAL;
    let isComplete = false;
...
const calculateChecklistStatus = (habit, log) => {
...
export const shouldShowHabitOnDate = (habit, date) => {
...
export const calculateHabitStats = (habit, allHabitLogs, today = new Date()) => {
    if (!habit) {
    ...
    if (habit.type === HabitType.COUNT || habit.type === HabitType.DURATION) {
        ...
        goalProgress = Array.from(logsByDate.values()).reduce((sum, log) => sum + (log.completedChecklistItems?.length || 0), 0);
    }
